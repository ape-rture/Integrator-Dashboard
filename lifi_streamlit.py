...
def load_data(integrator, start_date, end_date, transaction_status):
    ...
    if integrator_complete_df.empty:
...
# do date check, else load data
if start_date >= end_date:
    data_load_state.text("")
    st.warning('Please select a start date earlier then the before date!', icon="⚠️")
    ...
    if df.empty:
        st.warning('No data available for selected parameters. Try another combination.', icon="⚠️")
